...
class GetData:
    ...
    def get_local_authority_data(self, url_suffix='Information/MonitoringLocalAuthority/GroupName=London/Json',
        ...
        if self.authority_name is None:
            ...
            for item in data:
                if item['@LocalAuthorityName'] == self.authority_name and get_id_only == False:
                    data = item
                elif item['@LocalAuthorityName'] == self.authority_name and get_id_only == True:
    ...
    def get_daily_pollution_stats(self, local_authority_ids: List):
        """Provides daily pollution stats based on area: need to provide local_authority_ids in the for of a List"""
            ...
            try:
                ...
                for item in list_data:
                    if type(item['Species']) == list:
                        ...
                        daily_pollution_df = daily_pollution_df.append({'species_code': item['Species']['@SpeciesCode'],
                                                                        'site_code': item['@SiteCode'],
                                                                        'air_quality_index': int(item['Species'][
                                                                                                     '@AirQualityIndex']),
                ...
                for item in dict_data['Species']:
                    daily_pollution_df = daily_pollution_df.append({'species_code': item['@SpeciesCode'],
                                                                    ...
                                                                    'air_quality_index': int(item['@AirQualityIndex']),
                                                                    'air_quality_band': item['@AirQualityBand'],
                                                                    'site_name': dict_data['@SiteName'],
                                                                    "retrieval_timestamp": pd.to_datetime(
                                                                        'today').strftime('%Y-%m-%d')},
                                                                   ignore_index=True)
        daily_pollution_df['borough'], daily_pollution_df['site_name'] = daily_pollution_df['site_name'].str.split(
            ' - ', 1).str
        return daily_pollution_df

    @staticmethod
    def plot_pollution_by_species(species, data_df: pd.DataFrame):
        df_species = data_df.loc[(data_df['species_code'] == species) & (data_df['objective_name'] == 'Capture Rate (%)')]
        df_species['value'] = df_species['value'].apply(pd.to_numeric, errors='coerce')
